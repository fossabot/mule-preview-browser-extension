...
(defn tooltip [change-record labels location]
  [:div {:class "mp-popover"}
   [:h1 (str "Line " (:line location) ", Column " (:column location))]
...
(defn mule-component-inner [{:keys [name description css-class content-root location change-record showing-atom labels]}]
  (let [anchor-el (clojure.core/atom nil)
        ...
        should-show-tooltip (boolean (or change-record (:added labels) (:removed labels)))]
    (fn []
      [:div {:ref #(reset! anchor-el %)}
       [:div  {:class ["component-container" css-class]
               :on-mouse-enter (m/handler-fn (reset! showing-atom should-show-tooltip))
               :on-mouse-leave  (m/handler-fn (reset! showing-atom false))}
        (when diff-icon-url (image diff-icon-url "diff-icon" content-root))
        [:div
         {:class ["component" name]}
         [:> popper {:open @showing-atom :anchor-el @anchor-el :placement "right"} (r/as-element tooltip)]
